...
It includes some on board gesture detection (such as fall detection) in hardware, and additional gesture sensing (e.g. logo-up, logo-down, shake) via software algorithms.
...
The compass must be calibrated before use. The calibration process is automatically initiated by the runtime software.
